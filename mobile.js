...
document.addEventListener("DOMContentLoaded", function () {
  ...
  d3.csv("data/mobile_phone_use.csv").then(data => {
...
states.forEach(state => {
  ...
  const svg = d3.select("#pie-chart").html("")
    .append("svg")
    .attr("width", 800) // Width to accommodate legend
    .attr("height", 400);
  ...
  // *** Removed the horizontal text labels on pie slices ***
