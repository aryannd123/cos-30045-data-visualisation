...
document.addEventListener("DOMContentLoaded", function () {
  ...
  d3.csv("data/non_wearing_seatbelts.csv").then(data => {
...
states.forEach(state => {
  ...
  const svg = d3.select("#pie-chart")
    .html("")
    .append("svg")
    .attr("width", 600) // Increased to fit the legend
    .attr("height", 400);
  ...
  // Removed slice labels (g.selectAll("text"))

  // Add a legend to the right of the chart
  const legend = svg.append("g")
    .attr("transform", "translate(420, 40)");
